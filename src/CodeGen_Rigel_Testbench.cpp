...
class Rigel_Closure : public Closure {
public:
    Rigel_Closure(Stmt s)  {
        s.accept(this);
    }

    vector<Rigel_Argument> arguments(const Scope<CodeGen_Rigel_Base::Stencil_Type> &scope);
...
vector<Rigel_Argument> Rigel_Closure::arguments(const Scope<CodeGen_Rigel_Base::Stencil_Type> &streams_scope) {
    vector<Rigel_Argument> res;
    ...
    for (const pair<string, Type> &i : vars) {
        ...
        if(ends_with(i.first, ".stream") ||
           ends_with(i.first, ".stencil") ) {
            CodeGen_Rigel_Base::Stencil_Type stype = streams_scope.get(i.first);
            res.push_back({i.first, true, Type(), stype});
        } else if (ends_with(i.first, ".stencil_update")) {
            internal_error << "we don't expect to see a stencil_update type in Rigel_Closure.\n";
        } else {
            // it is a scalar variable
            res.push_back({i.first, false, i.second, CodeGen_Rigel_Base::Stencil_Type()});
        }
    }
    return res;
}
...
CodeGen_Rigel_Testbench::CodeGen_Rigel_Testbench(ostream &tb_stream)
    : CodeGen_Rigel_Base(tb_stream, CPlusPlusImplementation, ""),
...
CodeGen_Rigel_Testbench::~CodeGen_Rigel_Testbench() {
}

void CodeGen_Rigel_Testbench::visit(const ProducerConsumer *op) {
    if (starts_with(op->name, "_hls_target.")) {
        Stmt hw_body = op->produce;

        debug(1) << "compute the closure for " << op->name << '\n';
        Rigel_Closure c(hw_body);
        vector<Rigel_Argument> args = c.arguments(stencils);

        // generate Rigel target code using the child code generator
        ...
        CodeGen_Rigel_Base::visit(op);
    }
}

void CodeGen_Rigel_Testbench::visit(const Call *op) {
...
void CodeGen_Rigel_Testbench::visit(const Realize *op) {
...
void CodeGen_Rigel_Testbench::visit(const Block *op) {
