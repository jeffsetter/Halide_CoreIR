...
string CodeGen_Rigel_Base::print_stencil_type(Stencil_Type stencil_type) {
    ostringstream oss;
...
string CodeGen_Rigel_Base::print_name(const string &name) {
    ostringstream oss;
	...
	// vivado Rigel compiler doesn't like '__'
        if (!isalnum(name[i])) {
            oss << "_";
        }
        else oss << name[i];
...
string CodeGen_Rigel_Base::print_stencil_pragma(const string &name) {
    // nothing is printed by default
    return string();
}

void CodeGen_Rigel_Base::visit(const Call *op) {
...
void CodeGen_Rigel_Base::visit(const Realize *op) {
...
void CodeGen_Rigel_Base::visit(const Provide *op) {
