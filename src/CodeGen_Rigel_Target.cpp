...
CodeGen_Rigel_Target::CodeGen_Rigel_Target(const string &name)
    : target_name(name),
      hdrc(hdr_stream, CodeGen_Rigel_C::CPlusPlusHeader),
      srcc(src_stream, CodeGen_Rigel_C::CPlusPlusImplementation) { }


CodeGen_Rigel_Target::~CodeGen_Rigel_Target() {
    hdr_stream << "#endif\n";
...
void CodeGen_Rigel_Target::init_module() {
    debug(1) << "CodeGen_Rigel_Target::init_module\n";
...
void CodeGen_Rigel_Target::add_kernel(Stmt s,
                                    const string &name,
                                    const vector<Rigel_Argument> &args) {
    debug(1) << "CodeGen_Rigel_Target::add_kernel " << name << "\n";

    hdrc.add_kernel(s, name, args);
    srcc.add_kernel(s, name, args);
}

void CodeGen_Rigel_Target::dump() {
    std::cerr << src_stream.str() << std::endl;
}


string CodeGen_Rigel_Target::CodeGen_Rigel_C::print_stencil_pragma(const string &name) {
    ostringstream oss;
...
void CodeGen_Rigel_Target::CodeGen_Rigel_C::add_kernel(Stmt stmt,
                                                   const string &name,
                                                   const vector<Rigel_Argument> &args) {
    ...
    for (size_t i = 0; i < args.size(); i++) {
        ...
        if (args[i].is_stencil) {
            CodeGen_Rigel_Base::Stencil_Type stype = args[i].stencil_type;
...
void CodeGen_Rigel_Target::CodeGen_Rigel_C::visit(const For *op) {
...
void CodeGen_Rigel_Target::CodeGen_Rigel_C::visit(const Allocate *op) {
