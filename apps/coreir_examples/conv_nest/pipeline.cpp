...
public:
  ...
  RDom win, win_in;
    ...
    MyPipeline() : input(UInt(8), 2, "input"), 
                   kernel("kernel"), conv1("conv1"), conv2("conv2"),
                   output("output"), hw_output("hw_output"),
                   win(0, 3, 0, 3), win_in(0,4,0,4) {
	...
	conv1.update(0).unroll(win.x).unroll(win.y);
        ...
        // nest kernel
        conv2(x,y) += conv1(x+win_in.x, y+win_in.y) * kernel(win_in.x, win_in.y);
        conv2.update(0).unroll(win_in.x).unroll(win_in.y);
    ...
    void compile_cpu() {
        ...
        output.tile(x, y, xo, yo, xi, yi, 59,59);
    ...
    void compile_hls() {
        ...
        hw_output.tile(x, y, xo, yo, xi, yi, 59,59).reorder(xi, yi, xo, yo);
