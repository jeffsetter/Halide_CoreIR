...
public:
    ...
    void compile_hls() {
        ...
        hw_output.tile(x, y, xo, yo, xi, yi, 640, 480)
            .reorder(c, xi, yi, xo, yo);
        hw_output.unroll(c).unroll(xi, 2);          // 2 pix/cyc throughput
    ...
    void compile_coreir() {
        ...
        processed.compile_to_lowered_stmt("pipeline_coreir.ir.html", args, HTML, coreir_target);
        processed.compile_to_coreir("pipeline_coreir.cpp", args, "pipeline_coreir", coreir_target);
        processed.compile_to_header("pipeline_coreir.h", args, "pipeline_coreir", coreir_target);
...
int main(int argc, char **argv) {
    ...
    MyPipelineOpt p3(4, matrix, gamma, contrast);
    p3.compile_coreir();
    return 0;
}
