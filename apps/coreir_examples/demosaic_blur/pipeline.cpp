...
class MyPipeline {
 public:
        ...
        output("output"), hw_output("hw_output")
  {
    ...
    hw_output(x, y, c) = lowpass_x(c, x, y);
  ...
  void compile_cpu() {
    ...
    lowpass_x.compute_at(hw_output, xo)
        .vectorize(x, 8).unroll(c);



    //output.print_loop_nest();
    hw_output.compile_to_lowered_stmt("pipeline_native.ir.html", args, HTML);
    hw_output.compile_to_header("pipeline_native.h", args, "pipeline_native");
    hw_output.compile_to_object("pipeline_native.o", args, "pipeline_native");
  }
  ...
  void compile_hls() {
    ...
    hw_output.tile(x, y, xo, yo, xi, yi, 61, 61);
    ...
    hw_output.compile_to_lowered_stmt("pipeline_hls.ir.html", args, HTML, hls_target);
    hw_output.compile_to_hls("pipeline_hls.cpp", args, "pipeline_hls", hls_target);
    hw_output.compile_to_header("pipeline_hls.h", args, "pipeline_hls", hls_target);

    std::vector<Target::Feature> features({Target::Zynq});
    Target target(Target::Linux, Target::ARM, 32, features);
    hw_output.compile_to_zynq_c("pipeline_zynq.c", args, "pipeline_zynq", target);
    hw_output.compile_to_header("pipeline_zynq.h", args, "pipeline_zynq", target);
    hw_output.compile_to_lowered_stmt("pipeline_zynq.ir.html", args, HTML, target);
  }
  ...
  void compile_coreir() {
    ...
    hw_output.compile_to_lowered_stmt("pipeline_coreir.ir.html", args, HTML, coreir_target);
    hw_output.compile_to_coreir("pipeline_coreir.cpp", args, "pipeline_coreir", coreir_target);
  }
};
