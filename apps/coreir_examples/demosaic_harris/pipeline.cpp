...
class MyPipeline {
 public:
        ...
        maxWin(-1, 3, -1, 3)
  {
    ...
    {
      gray(x, y) = cast<uint8_t>((77 * cast<uint16_t>(lowpass_x(0, x, y))
                                  + 150 * cast<uint16_t>(lowpass_x(1, x, y))
                                  + 29 * cast<uint16_t>(lowpass_x(2, x, y))) >> 8);
  ...
  void compile_cpu() {
    ...
    /*
      ...
      .vectorize(x, 8);
    */
    lowpass_x.compute_at(output, xo).vectorize(x, 8).unroll(c);
  ...
  void compile_hls() {
    ...
    */
    ...
    hw_output.tile(x, y, xo, yo, xi, yi, 55,55)
        .reorder(c, xi, yi, xo, yo);
