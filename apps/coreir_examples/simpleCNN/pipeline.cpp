...
public:
    ...
    void compile_coreir() {
        ...
        // COREIR schedule: make a hw pipeline producing 'hw_output', taking
        // inputs of 'clamped', buffering intermediates at (output, xo) loop
        // level
        clamped.compute_root();
        hw_output.compute_root();
        ...
        //output.print_loop_nest();
        output.compile_to_lowered_stmt("pipeline_coreir.ir.html", args, HTML);
        output.compile_to_coreir("pipeline_coreir.cpp", args, "pipeline_coreir");
        output.compile_to_header("pipeline_coreir.h", args, "pipeline_coreir");
    }
