...
int main(int argc, char **argv) {
	...
	Image<int8_t> out_native(in.width(), in.height(), in.channels());
	Image<int8_t> out_hls(in.width(), in.height(), in.channels());
	Image<int8_t> out_coreir(in.width(), in.height(), in.channels());
	...
	for (int y = 0; y < in.height(); y++) {
		for (int x = 0; x < in.width(); x++) {
			for (int c = 0; c < in.channels(); c++) {
				...
				instream << state.getBitVec("self.in_arg_1_0_0") << " = " << state.getBitVec("self.in_arg_1_0_0").to_type<uint16_t>() << endl;
